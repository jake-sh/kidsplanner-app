...
self.addEventListener('fetch', e => {
  ...
  if (e.request.mode === 'navigate') {
    e.respondWith(fetch(e.request).catch(() => caches.match('/kidsplanner-app/index.html')));
    return;
  }
  // 나머지는 cache-first
  e.respondWith(
  ...
  e.waitUntil(
    ...
    self.clients.matchAll({ type: 'window', includeUncontrolled: true }).then(clients => {
      ...
      return self.registration.showNotification(data.title || '새 메시지', {
        ...
        icon: '/kidsplanner-app/icons/icon-192.png',
...
self.addEventListener('notificationclick', e => {
  ...
  e.waitUntil(
    self.clients.matchAll({ type: 'window', includeUncontrolled: true }).then(list => {
      ...
      return self.clients.openWindow('/kidsplanner-app/');
    })
  );
});
