...
// ── PATTERN ────────────────────────────────────────
// Kids Planner: 드래그 패턴 진입 제거됨. 메뉴는 onclick으로 직접 진입.
...
function openFeature(i) {
  ...
  else if (i === 4) openTodo();
  else if (i === 5) openStats();
  else if (i === 6) showScreen('contactScreen');
  else if (i === 7) enterChatApp();
  else if (i === 8) showScreen('albumScreen');
}
...
const planTitles = ['알림장','일정','알람','메모','할 일','통계','연락처','채팅','앨범'];
const planTitlesEn = ['Notice','Schedule','Alarm','Memo','To-Do','Stats','Contact','Chat','Album'];
const planTitlesZh = ['通知','日程','闹钟','备忘','待办','统计','联系人','聊天','相册'];
const planTitlesJa = ['お知らせ','予定','アラーム','メモ','タスク','統計','連絡先','チャット','アルバム'];
...
// ── Kids Planner: Enhanced Security 제거됨 ──────────────
function isAutoLockOn() { return false; } // 항상 OFF

// 자동잠금 관련 이벤트 모두 비활성화
var _appWasHidden = false;
var _filePickerOpen = false;

// 파일 선택창 열릴 때 플래그 (기존 동작 유지)
...
function applyDarkMode() {
  ...
  applyIconStyle();
  // Kids Planner: 채팅 테마도 메인 다크모드 따라가기
...
function renderTodoList() {
  ...
  el.innerHTML = todos.map((t,i) => {
    var medBadge = '';
    if (t.isMed) {
      var timeMap = { morning:'🌅아침', noon:'☀️점심', night:'🌙저녁' };
      var timeLabel = t.medTime ? (timeMap[t.medTime] || '') : '';
      medBadge = '<span style="display:inline-block;background:#fef3c7;color:#92400e;font-size:10px;font-weight:700;padding:2px 6px;border-radius:8px;margin-right:6px;">💊 투약 ' + timeLabel + '</span>';
    }
    return '<div class="todo-item ' + (t.done?'todo-done':'') + '">' +
      '<div class="todo-check ' + (t.done?'checked':'') + '" onclick="toggleTodo(' + i + ')">' + (t.done?'✓':'') + '</div>' +
      '<span class="todo-text">' + medBadge + esc(t.text) + '</span>' +
...
// 투약 체크박스 토글
function toggleMedOption() {
  var cb = document.getElementById('medCheck');
  var opts = document.getElementById('medTimeOptions');
  if (!cb || !opts) return;
  opts.style.display = cb.checked ? 'flex' : 'none';
  // 체크 해제 시 시간 선택도 초기화
  if (!cb.checked) {
    ['medMorning','medNoon','medNight'].forEach(function(id){
...
function addTodo() {
  ...
  var isMed = false;
  var medTimes = [];
  var medCheckEl = document.getElementById('medCheck');
  if (medCheckEl && medCheckEl.checked) {
    isMed = true;
    if (document.getElementById('medMorning')?.checked) medTimes.push('morning');
    if (document.getElementById('medNoon')?.checked) medTimes.push('noon');
    if (document.getElementById('medNight')?.checked) medTimes.push('night');
    // 시간 선택 없으면 일반 투약
    if (medTimes.length === 0) medTimes = [null];
  } else {
    medTimes = [null];
  }
  
  // 시간별로 분리 추가 (아침/저녁 둘 선택 시 2개 리스트업)
  if (isMed && medTimes.filter(t=>t).length > 0) {
    medTimes.forEach(function(t) {
      if (t) todos.unshift({ text: text, done: false, isMed: true, medTime: t });
    });
  } else {
    todos.unshift({ text: text, done: false, isMed: isMed });
  }
...
function applyChatTheme() {
  // Kids Planner: 메인 다크모드 설정을 따름
  const isDark = localStorage.getItem('darkMode') === 'true' || 
                 (localStorage.getItem('darkMode') === null && localStorage.getItem('chatTheme') === 'light' ? false : true);
  const chatApp = document.getElementById('chatApp');
  if (!chatApp) return;
  // 메인 다크모드 ON → 채팅도 다크 (chat-light 클래스 제거)
  // 메인 다크모드 OFF → 채팅도 라이트
  if (!isDark) {
...
async function sendNotification(title, body) {
  ...
  if (sw && sw.active) {
    ...
    new Notification(title, { body, icon: '/kidsplanner-app/icons/icon-192.png' });
  }
}
...
const I18N = {
  ko: {
    notice: '알림장', schedule: '일정', alarm: '알람',
    memo: '메모', todo: '할 일', stats: '통계',
    contact: '연락처', chat: '채팅', album: '앨범',
    // 호환성용 옛 키
    goal: '목표', project: '프로젝트', tag: '태그', calendar: '달력',
    settings: '설정', back: '← 뒤로',
  ...
  en: {
    notice: 'Notice', schedule: 'Schedule', alarm: 'Alarm',
    memo: 'Memo', todo: 'To-Do', stats: 'Stats',
    contact: 'Contact', chat: 'Chat', album: 'Album',
    // 호환성용 옛 키
    goal: 'Goals', project: 'Projects', tag: 'Tags', calendar: 'Calendar',
  ...
  zh: {
    notice: '通知', schedule: '日程', alarm: '闹钟',
    memo: '备忘', todo: '待办', stats: '统计',
    contact: '联系人', chat: '聊天', album: '相册',
    // 호환성용
    goal: '目标', project: '项目', tag: '标签', calendar: '日历',
    settings: '设置', back: '← 返回',
  ...
  ja: {
    notice: 'お知らせ', schedule: '予定', alarm: 'アラーム',
    memo: 'メモ', todo: 'タスク', stats: '統計',
    contact: '連絡先', chat: 'チャット', album: 'アルバム',
    // 호환성용
    goal: '目標', project: 'プロジェクト', tag: 'タグ', calendar: 'カレンダー',
    settings: '設定', back: '← 戻る',
...
function applyLang() {
  ...
  // 메인 메뉴 (Kids Planner: 알림장,일정,알람,메모,할일,통계,연락처,채팅,앨범)
  var menuLabels = document.querySelectorAll('#menuGrid .menu-label');
  var menuKeys = ['notice','schedule','alarm','memo','todo','stats','contact','chat','album'];
